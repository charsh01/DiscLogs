...
def search_data():
    ...
    def get_data(release):
            ...
            # TO DO: add condition for CDs to return different format value; currently returning 'album' and 'comp'
                format_list = []
                for x in release.formats[0]:
                    if x == "name":
                        continue
                    if x == "qty":
                        format_list.append(str(" ") + str(release.formats[0][x]) + "x" + str(release.formats[0]["descriptions"][0]))
                    if x == "descriptions":
                        try:
                            format_list.append(str(" ") + str(release.formats[0][x][2]))
                        except(IndexError):
                            continue
                    if x == "text":
                        format_list.append(str(" ") + str(release.formats[0][x]))
            except AttributeError:
                format = 'none'
            try:
                year = release.year
